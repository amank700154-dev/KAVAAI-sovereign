...
Model Router
============
Chooses the appropriate local AI model based on the user's task.

Example:
- reasoning task       -> reasoning model (qwen2.5:7b)
- coding task          -> coding model (qwen2.5:7b)
- image/document task  -> vision model (qwen2.5vl:7b)
- embeddings           -> embedding model (all-MiniLM-L6-v2)

Provides graceful deterministic fallback if local Ollama is offline.
"""
...
CONFIG_FILE = os.path.join(ROOT_DIR, "model_config.json")
